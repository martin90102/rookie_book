...
void SIFT_features(Mat &image, vector<KeyPoint> &dst)
{
    // 如果不是灰度图，转换为灰度图
    if (image.channels() != 1)
    {
...
void features(Mat &image, Mat &dst)
{
    ...
    // auto diff = end - start;
    // cout<<"harris time: "<<chrono::duration<double, milli>(diff).count()<<" ms"<<endl;
    
    Mat harris_dst = image.clone();
    drawKeypoints(image, keypoints_harris, harris_dst, Scalar::all(-1), DrawMatchesFlags::DEFAULT);
    putText(harris_dst, "Harris", Point(10, 30), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2, 8, 0);
    ...
    putText(orb_dst, "ORB", Point(10, 30), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2, 8, 0);
    ...
    putText(kaze_dst, "KAZE", Point(10, 30), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2, 8, 0);
    ...
    putText(akaze_dst, "AKAZE", Point(10, 30), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2, 8, 0);

    // sift
    Mat sift_dst = image.clone();
    vector<KeyPoint> keypoints_sift;
    SIFT_features(image, keypoints_sift);
    drawKeypoints(image, keypoints_sift, sift_dst, Scalar::all(-1), DrawMatchesFlags::DEFAULT);
    putText(sift_dst, "SIFT", Point(10, 30), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2, 8, 0);

    // stitch all images(2 rows,each row center aligned)
    Mat row1, row2, all;
    hconcat(harris_dst, orb_dst, row1);
    hconcat(row1, kaze_dst, row1);
    hconcat(akaze_dst, sift_dst, row2);
    // calculate the size for padding
    int pad = (row1.cols - row2.cols) / 2;
    // padding
    copyMakeBorder(row2, row2, 0, 0, pad, pad, BORDER_CONSTANT, Scalar(0, 0, 0));
    // concat
    vconcat(row1, row2, all);

    // save image
    imwrite("all_features.png", all);
}
